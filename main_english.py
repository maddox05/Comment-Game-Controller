...
def detect_language(comment):
    ...
    # Check which language keywords are found in the comment
    ...
    # Determine the language based on the matches
    if spanish_matches:
        return "spanish"
        ...
        return "spanish"  # Use Spanish as the default if no language is detected
...
def save_key_mapping_usage(key_mapping_usage):
    ...
    with open("log2.txt", "w") as log2_file:
        log2_file.write(f"Key Mapping Usage Record from {get_date_string(first_record_date)} - {get_date_string(current_date)}\n")
...
async def on_ttcomment(event: CommentEvent):
    try:
        ...
        if detected_language == "english":
            ...
            selected_key_mapping = key_mapping_spanish  # Use Spanish as the default
